...
public class MkguQuestions {
    ...
    public static MkguQuestionXmlRoot getQuestions(Map<String, String> mkguFormVersion, String text) {
        ...
        while (mkguQuestionnairesIterator.hasNext()) {
            ...
            if (mq.getVersion().equals(mkguFormVersion.get("version"))) {
                mkguQuestionXmlRoot.setVersion(mkguFormVersion.get("version"));
                xml = mq.getXml();
            }
        }
        ...
        try {
            ...
            Element blocks = rootElement.getChildren("blocks").get(0);
            mkguQuestionXmlRoot.setQuestionTitle(blocks.getChildren().get(0).getValue());
            Element indicators = rootElement.getChildren("indicators").get(0);
